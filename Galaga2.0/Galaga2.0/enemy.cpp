...
void Enemy::attackPlayer(sf::Time &timeElapsed)
{
	...
	//float enemy_x = rand() % (ENEMY_SPEED) + 1; 
	...
	if (sprite.getPosition().y > (GAME_HEIGHT / 4))
	{
		if (!hasFired)
		{
			fire(); // Fire three times
			hasFired = true;
		}
	}
}

//void Enemy::moveSequence(sf::ConvexShape &shape, sf::Time &timeElapsed, sf::Vector2f &velocity, int point_num, int select_mode, int point_total, double degrees_total)
void Enemy::moveSequence(sf::Time timeElapsed, sf::Vector2f &velocity, int point_num, int select_mode, int point_total, double degrees_total)
{
	...
	//shape.setPointCount(point_num + 1);
